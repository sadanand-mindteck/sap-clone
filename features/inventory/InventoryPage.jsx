  ...
  const columns = useMemo(() => [
      ...
      cell: info => {
        ...
        return (
          <span className={cn(
            ...
            val < 10 ? "bg-red-100 text-red-800" : "text-erp-800"
          )}>
            {val}
          </span>
        )
    ...
    },
    {
      ...
      cell: info => {
        ...
        return (
          <span className={cn("text-[10px] px-1.5 py-0.5 rounded border border-transparent font-medium", colors[status] || "bg-gray-100")}>
            {status.toUpperCase()}
          </span>
        );
      }
  ...
  return (
    <div className="h-full flex flex-col bg-white border border-erp-300 shadow-sm rounded-sm overflow-hidden">
        ...
        <div className="flex-1 overflow-auto">
          <table className="w-full border-collapse text-xs">
                  ...
                  {headerGroup.headers.map(header => (
                    ...
                    >
                      <div className="flex items-center">
                        ...
                        {header.column.getIsSorted() && <SortIcon className={header.column.getIsSorted() === 'desc' ? 'transform rotate-180' : ''} />}
                      </div>
                    </th>
                  ))}
                </tr>
            ...
            <tbody className="bg-white">
              {isLoading ? (
                <tr><td colSpan={columns.length} className="p-4 text-center">Loading data...</td></tr>
              ) : table.getRowModel().rows.length === 0 ? (
                <tr><td colSpan={columns.length} className="p-4 text-center text-erp-500">No records found.</td></tr>
              ) : (
      ...
      <div className="p-1 bg-erp-50 border-t border-erp-300 flex items-center justify-between text-xs text-erp-600">
        <div className="px-2">
          {table.getFilteredRowModel().rows.length} items loaded
        </div>
